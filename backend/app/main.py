...
from app.auth import pop_oauth_state
...
app.add_middleware(
    ...
    allow_origins=[
        "http://localhost:5173",
        "http://127.0.0.1:5173",
        "https://ai-agent-chatbot-tan.vercel.app",
    ],
...
def signup(request: Request, body: SignupRequest, response: Response, db: DBSession = Depends(get_db)):
...
@app.post("/auth/login")
@limiter.limit("5/minute")
def login(request: Request, body: SignupRequest, response: Response, db: DBSession = Depends(get_db)):
...
def finalize_signin(request: Request, token: str, response: Response, db: DBSession = Depends(get_db)):
    payload = pop_oauth_state(db, token)
    if not payload:
        raise HTTPException(status_code=400, detail="Invalid or expired sign-in token")

    session_id = create_session(db, payload["user_id"])
    response.set_cookie(key="session_id", value=session_id, httponly=True, secure=IS_PRODUCTION, samesite="lax", max_age=60 * 60 * 24 * 7)
    return {"ok": True}
